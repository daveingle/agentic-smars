...
impl PlanLibraryLoader {
    pub fn load_from_specs(specs_dir: &str) -> Result<PlanLibrary> {
        let mut builder = PlanLibraryBuilder::new(specs_dir);
...
impl PlanLibrary {
    pub fn search_plans(&self, query: &str) -> Vec<&SMARSPlan> {
        let query_lower = query.to_lowercase();
        self.plans.values()
            .filter(|plan| {
                plan.name.to_lowercase().contains(&query_lower) ||
                plan.description.to_lowercase().contains(&query_lower) ||
                plan.keywords.iter().any(|k| k.contains(&query_lower)) ||
                plan.domain.to_lowercase().contains(&query_lower)
            })
            .collect()
    }
    
    pub fn get_plans_by_domain(&self, domain: &str) -> Vec<&SMARSPlan> {
        self.plans.values()
            .filter(|plan| plan.domain == domain)
            .collect()
    }
    
    pub fn get_plans_by_complexity(&self, complexity: &str) -> Vec<&SMARSPlan> {
        self.plans.values()
            .filter(|plan| plan.complexity == complexity)
            .collect()
    }
}
...
mod tests {
    ...
    fn test_library_search() {
        ...
        let plan = SMARSPlan {
            name: "web_api_plan".to_string(),
            steps: vec!["create_api".to_string()],
            domain: "web_development".to_string(),
            complexity: "medium".to_string(),
            source_file: "test.smars.md".to_string(),
            description: "API development plan".to_string(),
            keywords: vec!["api".to_string(), "web".to_string()],
            estimated_duration_minutes: 60,
            requires_user_input: false,
            artifacts_created: vec!["api_specification".to_string()],
        };
        
        library.plans.insert("web_api_plan".to_string(), plan);
        
        let results = library.search_plans("api");
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].name, "web_api_plan");
    }
}