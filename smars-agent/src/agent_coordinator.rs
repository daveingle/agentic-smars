...
impl AgentCoordinator {
    ...
    pub async fn coordinate_execution(&mut self, execution_context: &ExecutionContext) -> Result<CoordinationResult> {
        let coordination_id = Uuid::new_v4().to_string();
        
        println!("🤖 Starting multi-agent coordination for execution: {}", execution_context.execution_id);
        
        // Step 1: Identify required capabilities
        let required_capabilities = self.identify_required_capabilities(execution_context);
        ...
        for agent_id in &participating_agents {
            if let Some(agent) = self.agents.get(agent_id) {
                let recommendation = self.get_agent_recommendation(agent, execution_context).await?;
    ...
    pub fn get_active_agents(&self) -> Vec<&Agent> {
        self.agents.values().filter(|agent| agent.active).collect()
    }
    
    pub fn spawn_agent(&mut self, capabilities: &[String]) -> Result<String> {
        let agent_id = format!("agent-{}", Uuid::new_v4());
        
        // Determine agent type based on capabilities
        let agent_type = if capabilities.iter().any(|c| c.contains("validation")) {
            AgentType::ValidationAgent
        } else if capabilities.iter().any(|c| c.contains("simulation")) {
            AgentType::SimulationAgent
        } else if capabilities.iter().any(|c| c.contains("decision")) {
        ...
        let agent = Agent {
            ...
            confidence_level: 0.7, // New agents start with moderate confidence
            active: true,
        };
        
        self.agents.insert(agent_id.clone(), agent);
        
        println!("🤖 Spawned new agent: {} with capabilities: {:?}", agent_id, capabilities);
    ...
    fn identify_required_capabilities(&self, execution_context: &ExecutionContext) -> Vec<String> {
        let mut capabilities = Vec::new();
        
        // Analyze execution context to determine needed capabilities
        if !execution_context.success || execution_context.confidence_level < 0.7 {
            capabilities.push("validation".to_string());
        }
        
        if execution_context.requires_multi_agent_coordination {
            capabilities.push("coordination".to_string());
            capabilities.push("decision_making".to_string());
        }
        
        if execution_context.artifacts_created.is_empty() {
            capabilities.push("execution".to_string());
        }
        
        // Always include simulation for complex scenarios
        if execution_context.steps_completed.len() > 3 {
    ...
    async fn get_agent_recommendation(&self, agent: &Agent, execution_context: &ExecutionContext) -> Result<String> {
        ...
        let recommendation = match agent.agent_type {
            ...
            AgentType::SimulationAgent => {
                if execution_context.confidence_level < 0.6 {
            ...
            AgentType::DecisionAgent => {
                if execution_context.requires_user_input {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_agent_coordinator_initialization() {
        let coordinator = AgentCoordinator::new().await.unwrap();
        assert_eq!(coordinator.agent_count(), 3); // validator, simulator, decision
        
        let active_agents = coordinator.get_active_agents();
        assert_eq!(active_agents.len(), 3);
    }
    
    #[tokio::test]
    async fn test_agent_spawning() {
        let mut coordinator = AgentCoordinator::new().await.unwrap();
        let initial_count = coordinator.agent_count();
        
        let agent_id = coordinator.spawn_agent(&["custom_capability".to_string()]).unwrap();
        assert!(!agent_id.is_empty());
        assert_eq!(coordinator.agent_count(), initial_count + 1);
    }
    
    #[test]
    fn test_capability_matching() {
        let mut coordinator = AgentCoordinator {
            agents: HashMap::new(),
            coordination_history: Vec::new(),
            statistics: CoordinationStatistics {
                messages_exchanged: 0,
        ...
        let agent = Agent {
            ...
            agent_type: AgentType::ValidationAgent,
            capabilities: vec!["validation".to_string(), "analysis".to_string()],
            confidence_level: 0.8,
            active: true,
        };
        
        coordinator.agents.insert("test-agent".to_string(), agent);
        
        let selected = coordinator.select_agents_by_capabilities(&["validation".to_string()]);
        assert_eq!(selected.len(), 1);
        assert_eq!(selected[0], "test-agent");
    }
}